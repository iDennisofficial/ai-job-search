...
The job portal CLIs (the country-agnostic `linkedin-search` and `freehire-search` tools) are written in TypeScript and run with Bun.
...
If your local job board isn't covered by the shipped CLIs, generate an equivalent search skill with `/add-portal` — it scaffolds the same CLI structure for any public portal and test-runs a live query before registering. See the "Job search tools" section in the README.
