...
- **Tool execution boundaries**: the DeepSeek runner executes a fixed set of shell steps itself (LaTeX compiles, `pdftotext`, the Bun portal CLIs); no arbitrary shell is handed to the model. The `security-guards` CI job fails any PR that adds package-manifest lifecycle scripts or weakens the personal-data gitignore rules. The model writes files only through the runner's explicit `FILE:` output contract.
